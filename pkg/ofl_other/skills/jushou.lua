...
Fk:loadTranslationTable{
  ...
  [':ofl__jushou'] = '结束阶段，你可以翻面并摸X张牌（X为存活角色数），然后你可以令所有角色翻面并各摸三张牌，若如此做，你弃置场上所有装备牌，失去〖据守〗，获得〖突围〗。',
}
...
jushou:addEffect(fk.EventPhaseEnd, {
  ...
  on_use = function(self, event, target, player)
    ...
    player:turnOver()
    if player.dead then return end
    if room:askToSkillInvoke(player, { skill_name = jushou.name, prompt = "#ofl__jushou-invoke" }) then
