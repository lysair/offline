...
zhenjue:addEffect(fk.EventPhaseStart, {
  ...
  can_trigger = function(self, event, target, player)
    return player:hasSkill(zhenjue.name) and target.phase == Player.Finish and player:isKongcheng() and not target.dead
  end,
  on_cost = function(self, event, target, player)
    if player.room:askToSkillInvoke(player, {skill_name = zhenjue.name, prompt = "#zhenjue-invoke::"..target.id}) then
  ...
  on_use = function(self, event, target, player)
    ...
    if target:isNude() or
      #room:askToDiscard(target, {
        ...
        cancelable = true,
        prompt = "#zhenjue-discard:"..player.id}) == 0 then
