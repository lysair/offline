...
mingjian:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player)
    ...
    local choice = room:askToChoice(player, {
      ...
      prompt = "#ofl__mingjian-invoke::" .. target.id,
    })
    if #choice == 1 then
      event:setCostData(self, {tos = {target.id}, cards = listCards[U.ConvertSuit(choice[1], "sym", "int")]})
      return true
    end
  end,
  on_use = function(self, event, target, player)
    ...
    if #cards > 0 then
      room:moveCardTo(cards, Card.PlayerHand, target, fk.ReasonGive, mingjian.name, nil, true, player.id)
    end
  end,
})
