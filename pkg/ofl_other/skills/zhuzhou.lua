...
zhuzhou:addEffect(fk.Damage, {
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(zhuzhou.name) and
      target and not target.dead and target ~= data.to and
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:delay(500)
    room:doIndicate(target, {data.to})
    ...
    room:moveCardTo(id, Card.PlayerHand, target, fk.ReasonPrey, zhuzhou.name, nil, false, target)
  end,
})
