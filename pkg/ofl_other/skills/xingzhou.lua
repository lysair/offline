...
Fk:loadTranslationTable{
  ...
  ['lieshen'] = '列神',
  [':xingzhou'] = '每回合限一次，当手牌数最少的角色受到伤害后，你可以弃置两张手牌，视为对伤害来源使用一张【杀】，然后若其死亡，〖列神〗视为未发动过。',
}

xingzhou:addEffect(fk.Damaged, {
  anim_type = "offensive",
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(xingzhou.name) and data.from and not data.from.dead and data.from ~= player and
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:throwCard(event:getCostData(self), xingzhou.name, player, player)
