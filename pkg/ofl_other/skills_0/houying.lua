...
houying:addEffect('viewas', {
  ...
  before_use = function(self, player, use)
    use.extraUse = true
    player.room:throwCard(event:getCostData(self), houying.name, player, player)
  end,
...
houying:addEffect('targetmod', {
  bypass_times = function(self, player, skill, scope, card)
    return skill.trueName == "slash_skill" and scope == Player.HistoryPhase and card and table.contains(card.skillNames, houying.name)
  end,
})
