...
Fk:loadTranslationTable{
  ...
  [':ofl__fangtong'] = '出牌阶段结束时，若有“方”，你可以重铸一张手牌，若你重铸的牌与你的任意“方”点数之和为36，你可以将对应的“方”置入弃牌堆，然后对一名其他角色造成3点雷电伤害。',
...
fangtong:addEffect(fk.EventPhaseEnd, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(fangtong.name) and player.phase == Player.Play
      and #player:getPile("ofl__godzhangliang_fang") > 0 and not player:isKongcheng()
  end,
  ...
  on_use = function(self, event, target, player, data)
    ...
    if success and dat then
      room:moveCardTo(dat.cards, Card.DiscardPile, nil, fk.ReasonPutIntoDiscardPile, fangtong.name, nil, true, player.id)
