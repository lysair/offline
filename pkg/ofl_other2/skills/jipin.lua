...
jipin:addEffect(fk.Damage, {
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(jipin.name) and player:getHandcardNum() < data.to:getHandcardNum() and not data.to.dead
  end,
  on_cost = function(self, event, target, player, data)
    if player.room:askToSkillInvoke(target, {skill_name = jipin.name, prompt = "#ofl__jipin-invoke::" .. data.to.id}) then
      event:setCostData(player, {tos = {data.to}})
      return true
    end
  end,
  on_use = function(self, event, target, player, data)
    ...
    room:moveCardTo(card, Card.PlayerHand, target, fk.ReasonPrey, jipin.name, nil, false, target.id)
    ...
    local to = room:askToChoosePlayers(player, {
      ...
      skill_name = jipin.name})
    if #to > 0 then
      room:moveCardTo(card, Card.PlayerHand, to[1], fk.ReasonGive, jipin.name, nil, false, target.id)
    end
  end,
})
