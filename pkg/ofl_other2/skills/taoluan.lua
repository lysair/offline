...
Fk:loadTranslationTable{
  ['ofl__taoluan'] = '滔乱',
  ['#ofl__taol__taoluan'] = '滔乱：你可以将一张牌当任意基本牌或普通锦囊牌使用',
  ['@@changshi'] = '常侍',
...
taoluan:addEffect('viewas', {
  ...
  interaction = function(self, player)
    ...
    return U.CardNameBox {
      choices = U.getViewAsCardNames(player, taoluan.name, all_names, nil, player:getTableMark("ofl__taoluan-turn")),
      all_choices = all_names,
      default_choice = skill.name,
    }
  end,
  handly_pile = true,
  card_filter = function(self, player, to_select, selected)
    return #selected == 0 and Fk.all_card_types[skill.interaction.data] ~= nil
  end,
