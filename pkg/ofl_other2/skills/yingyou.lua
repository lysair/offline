...
yingyou:addEffect(fk.AfterCardsMove, {
  anim_type = "drawcard",
  can_trigger = function(self, event, target, player)
    if player:hasSkill(yingyou) and #player:getPile("$yingtian_ambition") > 0 and player:getMark("yingtian_ambition_shown") ~= 0 then
      for _, move in ipairs(player.room.data) do
