...
jukou:addEffect("active", {
  ...
  card_num = 0,
  target_num = 1,
  interaction = UI.ComboBox {choices = {"ofl__jukou1", "ofl__jukou2"}},
  ...
  target_filter = function(self, player, to_select, selected, cards)
    if #selected == 0 then
      ...
      elseif self.interaction.data == "ofl__jukou2" then
        for _, ids in pairs(to_select.special_cards) do
          if #ids > 0 then
            return true
          end
        end
  ...
  on_use = function(self, room, effect)
    ...
    elseif self.interaction.data == "ofl__jukou2" then
      ...
      room:moveCardTo(cards, Card.PlayerHand, target, fk.ReasonJustMove, jukou.name, nil, false, target)
    end
  end,
})
...
jukou:addEffect("prohibit", {
  prohibit_use = function(self, player, card)
    if player:getMark("@@ofl__jukou1-turn") > 0 and card and card.trueName == "slash" then
      return true
    end
    if player:getMark("@@ofl__jukou2-turn") > 0 and card then
