...
tuwei:addEffect("active", {
  ...
  target_filter = function (self, player, to_select, selected, selected_cards)
    return #selected == 0 and #selected_cards == 1 and
      to_select:hasEmptyEquipSlot(Fk:getCardById(selected_cards[1]).sub_type) and
      not table.contains(player:getTableMark(tuwei.name), to_select.id)
  end,
  on_use = function(self, room, effect)
    ...
    local choice = room:askToChoice(player, {
      choices = {"ofl__tuwei_draw", "ofl__tuwei_damage", "Cancel"},
...
tuwei:addLoseEffect(function (self, player, is_death)
  player.room:setPlayerMark(player, tuwei.name, 0)
end)
