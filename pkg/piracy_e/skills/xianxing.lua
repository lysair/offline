...
xianxing:addEffect(fk.TargetSpecifying, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(xianxing.name) and
      data.card.is_damage_card and player.phase == Player.Play and
...
xianxing:addEffect(fk.CardUseFinished, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:usedSkillTimes(xianxing.name, Player.HistoryTurn) > 1 and
      data.extra_data and data.extra_data.xianxing == player.id and not data.damageDealt and not player.dead
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local n = player:usedSkillTimes(xianxing.name, Player.HistoryTurn) - 1
    ...
    if choice == "xianxing_invalid" then
      ...
      room:loseHp(player, n, xianxing.name)
    end
  end,
})
