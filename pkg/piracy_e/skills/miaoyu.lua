...
miaoyu:addEffect(fk.HpRecover, {
  ...
  on_cost = function(self, event, target, player, data)
    if player.room:askToSkillInvoke(player, {
      ...
      prompt = "#ofl__miaoyu-invoke::" .. target.id,
    }) then
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:moveCardTo(room:getNCards(1), Card.PlayerHand, target, fk.ReasonGive, miaoyu.name, nil, false, player)
  end,
})
  ...
  on_use = function(self, event, target, player, data)
    ...
    for _, p in ipairs(room:getAlivePlayers()) do
      if not p.dead and p:getMark("@ofl__miaoyu-turn") > 0 then
        room:loseHp(p, player:usedSkillTimes(miaoyu.name, Player.HistoryTurn), miaoyu.name)
      end
    end
  end,
})
