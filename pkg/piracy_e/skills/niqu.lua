...
local spec = {
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(niqu.name) and data.card.trueName == "jink" and
      player:usedSkillTimes(niqu.name, Player.HistoryTurn) == 0
  end,
...
niqu:addEffect(fk.CardUseFinished, spec)
niqu:addEffect(fk.CardRespondFinished, spec)
