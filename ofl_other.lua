...
--官盗E3至臻·权谋：纪灵
local jiling = General(extension, "ofl__jiling", "qun", 4)
local shuangren = fk.CreateActiveSkill{
  name = "ofl__shuangren",
  anim_type = "offensive",
  card_num = 0,
  target_num = 1,
  ...
  on_use = function(self, room, effect)
    ...
    if pindian.results[target.id].winner == player then
      ...
      for i = 1, n, 1 do
        if player.dead then return end
        if not U.askForUseVirtualCard(room, player, "slash", nil, self.name,
          "#ofl__shuangren-slash:::"..i..":"..n, true, true, false, true) then
          break
        end
      end
    else
...
local shuangren_filter = fk.CreateFilterSkill{
...
shuangren:addRelatedSkill(shuangren_filter)
